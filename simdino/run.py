...
from data import epoch_indices, image_files, load_views
...
def parse_args():
    ...
    if args.workers < 1 or config.eps <= 0 or config.warmup < 0:
    ...
    with ThreadPoolExecutor(args.workers) as pool:
        with (args.out / "metrics.jsonl").open("a") as log:
            for iteration in range(start, config.steps):
                ...
                indices = rng.choice(len(files), config.batch, replace=False)
                if args.sampling == "epoch":
                    indices = epoch_indices(len(files), config.batch, args.seed, iteration)
                seeds = rng.integers(2**32, size=config.batch)
                items = [(files[i][0], int(seed)) for i, seed in zip(indices, seeds)]
                examples = pool.map(lambda item: load_views(item, config.size), items)
                views = np.stack(list(examples), axis=1)
                state, metrics = step(state, jax.device_put(views, batch_sharding))
                metrics = {key: float(value) for key, value in jax.device_get(metrics).items()}
                if not all(math.isfinite(value) for value in metrics.values()):
                    raise FloatingPointError(f"Non-finite metrics at step {iteration}: {metrics}")
                metrics.update(step=iteration + 1, seconds=time.perf_counter() - tick)
                log.write(json.dumps(metrics) + "\n")
                log.flush()
                if iteration == start or (iteration + 1) % 10 == 0:
                    print(json.dumps(metrics), flush=True)
                if (iteration + 1) % 100 == 0 or iteration + 1 == config.steps:
