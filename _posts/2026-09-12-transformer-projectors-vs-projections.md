...
It also turns out to be the handiest way I know to think about extending a pretrained model. Retrieved documents, a bolted-on vision encoder, or a reused layer all get much easier to reason about once you separate what the model learned from what you hand it at inference time.
...
The $k$-NN view only explains the per-layer aggregation. MLPs store learned associations too, and probing them directly {% cite geva2021transformer %} shows that the two feed-forward matrices behave like a static key-value memory of their own.
...
Getting the dimensions to match is only half of it, since training still has to make the encoder's features useful to the language model. For example, original LLaVA first trains the adapter with both pretrained models frozen, then tunes the adapter and language model on visual instructions while keeping the vision encoder frozen {% cite liu2023visual %}. In the same type-based view, the vision transformer produces `Type[Visual]` tokens, and the adapter maps them to `Type[LinguoVisual]`. With alignment training, the LLM learns to accept both language and adapted visual tokens, broadening its input type from `Type[Language]` to `Type[Union[Language, LinguoVisual]]`. So the common token interface makes composition easy to wire up, and alignment training makes it actually work.
