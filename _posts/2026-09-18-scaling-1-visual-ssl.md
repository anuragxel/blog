...
The loss has the functional form of InfoNCE {% cite oord2018representation %}. Under its usual sampling assumptions, minimizing InfoNCE maximizes a lower bound on mutual information between the two views. With $2B-1$ candidates, that bound cannot exceed $\log(2B-1)$. I read this as motivation for SimCLR's batch-size results, not a full explanation. Empirically, bigger batches (thousands of images) bring more in-batch negatives, and SimCLR liked that.

There are three design decisions in SimCLR I like thinking about:

**Augmentations are the supervision.** The loss says, "Learn a representation that is invariant to the augmentation pipeline." Random crop implies global identity is not sensitive to individual regions or pixels. Color jitter implies global identity is not encoded in color statistics. This is still the pretext-task guess, just stated as an invariance in the latent space instead of pixel space, and that turns out to be far more robust.
...
Balancing these two heuristics is tricky and fragile. There's also the headache of tuning the EMA schedule, the temperature schedules, weight-decay schedules, and the last-layer freezing tricks. While the method works spectacularly (the emergent attention-map segmentation in DINO and the DINOv2 {% cite oquab2024dinov2 %} features are absolutely beautiful, especially with registers {% cite darcet2024vision %}), it is a stack of empirically discovered training stability tricks. The DINO paper shows empirically that you need both centering and sharpening to avoid the two forms of collapse, but as far as I know, why the combination works is still not fully settled.
...
## Shortcuts and collapse

Across all three families, the same two problems keep coming back: the network finding shortcuts, and the representation collapsing. Contrastive methods fight collapse with negatives, masked image modeling sidesteps it by predicting the data itself, and DINO leans on asymmetry plus a pile of tricks (or, in SimDINO, a direct penalty). The DINO family has shown particularly strong results at scale, as we've seen with Meta's [DINOv3](https://ai.meta.com/research/dinov3/).

Understanding and customizing these algorithms is useful, but scaling up the model and dataset brings another challenge: distributing the training job across many devices. In the next post, we’ll look at how to distribute any type of model training across devices.
