...
The $k$-NN view explains the per-layer aggregation operation. The model's stored knowledge sits in the projectors and (mostly) in the MLPs, which the $k$-NN view does not address. The "non-parametric" label from earlier in the series is about the per-layer aggregation rule, not the model as a whole.
...
That separation is consistent with models calling tools they have not seen at training time, as long as the new tool descriptions live in the context. Generalization to unseen tool schemas is real but bumpy and format-sensitive in practice, so the picture explains why transfer happens at all rather than predicting that it always works.
